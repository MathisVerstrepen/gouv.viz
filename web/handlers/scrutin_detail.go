...
func ScrutinDetail(ctx echo.Context) error {
	page, err := loadScrutinDetailPage(os.Getenv("DATABASE_PATH"), ctx.Param("uid"))
	if err != nil {
...
func loadScrutinDetailPage(databasePath string, uid string) (components.ScrutinDetailPage, error) {
	db, err := sql.Open("sqlite", databasePath)
	if err != nil {
		return components.ScrutinDetailPage{}, fmt.Errorf("open sqlite database: %w", err)
	}
	defer db.Close()
